...
def test_list_archives_reports_empty_state(template_copy) -> None:
    out = run_cmd(template_copy, "list-archives")
    assert out.stdout.strip() == "No reset archives found under state/reset_archives."


def test_list_archives_reports_manifest_and_legacy_archives(template_copy) -> None:
    _seed_runtime_artifacts_for_reset(template_copy)
...
def test_restore_restores_manifest_archive_over_existing_runtime_state(template_copy) -> None:
    _seed_runtime_artifacts_for_reset(template_copy)
    ...
    assert "Overwritten artifacts:" in out.stdout
...
def test_restore_restores_manifestless_legacy_archive(template_copy) -> None:
    _seed_runtime_artifacts_for_reset(template_copy)
...
def test_restore_rejects_broken_archive_without_mutating_runtime_state(template_copy) -> None:
    _seed_runtime_artifacts_for_reset(template_copy)
...
def test_prune_archives_requires_yes_in_non_interactive_mode(template_copy) -> None:
    _seed_runtime_artifacts_for_reset(template_copy)
    run_cmd(template_copy, "reset", "--yes")
...
def test_prune_archives_applies_keep_last_and_age_rules(template_copy) -> None:
    _seed_runtime_artifacts_for_reset(template_copy)
    run_cmd(template_copy, "reset", "--yes")
    archives_root = template_copy / "state" / "reset_archives"
    ...
    assert f"- {new_archive.name}" in out.stdout
    assert f"- {legacy_archive.name}" in out.stdout
