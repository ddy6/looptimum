...
def _build_suggestion_bundle(suggestions: list[JSONDict]) -> JSONDict:
    if not suggestions:
...
def _emit_suggest_output(
    ...
    payload: JSONDict = (
        suggestions[0] if len(suggestions) == 1 else _build_suggestion_bundle(suggestions)
    )
...
def _ensure_pending_manifest(
    paths: PathMap, pending_entry: JSONDict, *, objective_cfg: JSONDict, now: float
) -> None:
...
def _import_report_path(paths: PathMap, *, imported_at: float) -> Path:
...
def _build_report_payload(
    *,
    ...
    objective = cast(JSONDict, objective_cfg["primary_objective"])
    ...
    ranked_ok = sorted(
        ok_observations,
        key=lambda obs: best_rank_key(
            cast(JSONDict, obs["objectives"]),
            objective_cfg,
    ...
    top_trials = [
        _build_observation_report_row(obs, objective_cfg, include_params=True)
        for obs in ranked_ok[: max(1, int(top_n))]
    ]
    terminal_trials = [
        _build_observation_report_row(obs, objective_cfg, include_params=False)
        for obs in sorted(observations, key=lambda row: int(row.get("trial_id", 0)))
        if str(obs.get("status", "")) in {"failed", "killed", "timeout"}
    ]
    objective_trace = [
        _build_observation_report_row(obs, objective_cfg, include_params=False)
        for obs in sorted(observations, key=lambda row: int(row.get("trial_id", 0)))
    ]
    pareto_records = pareto_front_records(
        [cast(JSONDict, obs) for obs in ok_observations],
        objective_cfg,
    )
    ...
    report = {
        ...
        "pareto_front": {
            ...
            "trial_ids": [int(obs["trial_id"]) for obs in pareto_records],
            "trials": [
                _build_observation_report_row(obs, objective_cfg, include_params=True)
                for obs in pareto_records
            ],
        },
...
def _validate_trial_manifests_hard(trials_dir: Path, objective_cfg: JSONDict) -> list[str]:
    ...
    with hold_exclusive_lock(
        ...
        try:
            ...
            if import_mode == "strict":
                ...
                permissive_result = cast(
                    JSONDict,
                    normalize_import_records_permissive(
                        raw_rows,
                        row_format=row_format,
                        params=params,
                        objective_cfg=obj_cfg,
                ...
                normalized_rows = cast(list[JSONDict], permissive_result["accepted"])
                rejected_rows = cast(list[JSONDict], permissive_result["rejected"])
                ...
                report_payload: JSONDict = {
                    ...
                    "accepted_trial_ids": [
                        int(cast(JSONDict, record["observation"])["trial_id"])
            ...
            imported_observations = [
                cast(JSONDict, record["observation"]) for record in normalized_rows
            ]
            ...
            for observation, record in zip(imported_observations, normalized_rows, strict=True):
                source_trial_id = cast(int | str | None, record.get("source_trial_id"))
                if source_trial_id is not None:
...
def cmd_export_observations(args: argparse.Namespace) -> None:
    ...
    with hold_exclusive_lock(
        ...
        try:
            state = load_state(paths["state_file"])
            observations = cast(list[JSONDict], state.get("observations", []))
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    print(
        json.dumps(
            _status_payload(root, state, paths, max_pending_age, worker_leases_enabled),
            indent=2,
        )
    )
