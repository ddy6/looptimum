...
def propose(
    ...
    direction = str(scalarized_direction(obj_cfg))
...
def _suggest_telemetry_payload(latency_seconds: float) -> dict:
...
def _path_presence_payload(root: Path, paths: dict[str, Path]) -> dict:
    return {
        key: {
...
def _health_state(
    *, hard_errors: list[str], warnings_out: list[str], governance: dict | None
) -> str:
    if hard_errors:
        return "error"
...
def _build_health_payload(args: argparse.Namespace) -> tuple[dict, int]:
    ...
    lock_state = inspect_lock_state(paths["lock_file"])
...
def _build_metrics_payload(args: argparse.Namespace) -> tuple[dict, int]:
    ...
    if isinstance(status_payload, dict):
        ...
        observations_by_status = status_payload.get(
    ...
    if not health_payload["hard_errors"]:
        try:
            latency_summary = summarize_suggestion_latency(paths["acquisition_log_file"])
...
def cmd_suggest(args: argparse.Namespace) -> None:
    ...
    with hold_exclusive_lock(
        ...
        try:
            ...
            planned_decisions: list[tuple[int, dict, float]] = []
            try:
