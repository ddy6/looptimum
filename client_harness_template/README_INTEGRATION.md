...
- `objective.py`: client-fill stub. Replace `evaluate(params)` with your real evaluation code.
- `run_one_eval.py`: thin adapter that reads a suggestion file and writes an ingest-ready result JSON.
...
The optimization harness objective direction is defined in `objective_schema.yaml` (for example, `loss` + `minimize`).

Your integration must return the raw scalar in the same direction convention expected by the harness.
...
When an evaluation fails, still produce a result payload so the optimization loop can continue and the failure is recorded.
...
- infrastructure/transient errors (node preemption, network, scheduler issues)
...
4. Log detailed error context in your local system logs (not required in the ingest payload).
...
- If needed, extend your result schema and harness ingest validation in the template copy (optional).
