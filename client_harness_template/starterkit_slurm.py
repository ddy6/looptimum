...
from starterkit_scheduler import (
    ...
    build_slurm_array_submit_command,
    build_suggest_jsonl_command,
    render_shell_command,
)


def build_slurm_commands(
    *,
    project_root: Path,
    ...
    submit_command = build_slurm_array_submit_command(script_path=array_script_path, count=count)
    ...
    worker_shell = render_shell_command(worker_command)
    worker_shell = f'{worker_shell} --worker-index "${{SLURM_ARRAY_TASK_ID}}"'

    return {
        "controller_shell": f"{render_shell_command(controller_command)} > {shlex.quote(str(suggestions_file))}",
        "submit_shell": render_shell_command(submit_command),
...
def render_slurm_array_script(
    *,
    ...
    commands = build_slurm_commands(
        project_root=project_root,
        suggestions_file=suggestions_file,
        array_script_path=Path("starterkit_worker_array.sh"),
        count=count,
        work_dir=work_dir,
        python_executable=python_executable,
    ...
    lines = [
        ...
        f"# {commands['controller_shell']}",
        "# Submission step:",
        f"# {commands['submit_shell']}",
        "",
        commands["worker_shell"],
        "",
    ]
    return "\n".join(lines)
